...
Size
SharedDataShmemSize(void)
{
	Size		size;
	size	=	sizeof(MySharedData);
	size	=   add_size(size, TXN_SIZE * sizeof(WaitedNode));
	size	=   add_size(size, TXN_SIZE * sizeof(WaitingNode));
	size	=   add_size(size, TXN_SIZE * sizeof(PidCountNode));
	return 		size;
}
...
{	
	Size			size;
	size	=		sizeof(PC3Cache);
	size 	= 		add_size(size, sizeof(PC3HashKey));
	return size;
}

void
SharedDataShmemInit(void)
{
	bool		foundSharedData;
	sharedData = (struct MySharedData*)ShmemInitStruct("MySharedData", SharedDataShmemSize(), &foundSharedData);
	if(!foundSharedData)
	{		
		sharedData->conflict_num	=	0;

		// LWLockInitialize(&sharedData->waited_writeLock, LWTRANCHE_BUFFER_CONTENT);
		// LWLockInitialize(&sharedData->waiting_writeLock, LWTRANCHE_BUFFER_CONTENT);
		// LWLockInitialize(&sharedData->sleep_writeLock, LWTRANCHE_BUFFER_CONTENT);
		// LWLockInitialize(&sharedData->pid_writeLock, LWTRANCHE_BUFFER_CONTENT);
		
		for(int i=0; i<TXN_SIZE; i++)
		{
			sharedData->waiting_Vxids[i]										=	(WaitingNode*)ShmemAlloc(sizeof(WaitingNode));
			sharedData->waiting_Vxids[i]->vxid.backendId 						= 	InvalidBackendId;
			sharedData->waiting_Vxids[i]->vxid.localTransactionId				=	InvalidLocalTransactionId;
			sharedData->waiting_Vxids[i]->destination_vxid.backendId 			= 	InvalidBackendId;
			sharedData->waiting_Vxids[i]->destination_vxid.localTransactionId	=	InvalidLocalTransactionId;
			sharedData->waiting_Vxids[i]->isTheLast								=	false;

			sharedData->waited_Vxids[i]									=	(WaitedNode*)ShmemAlloc(sizeof(WaitedNode));
			sharedData->waited_Vxids[i]->pid							=	0;
			sharedData->waited_Vxids[i]->isTheLast						=	false;
			sharedData->waited_Vxids[i]->vxid.backendId 				= 	InvalidBackendId;
			sharedData->waited_Vxids[i]->vxid.localTransactionId		=	InvalidLocalTransactionId;
			sharedData->waited_Vxids[i]->source_vxid.backendId 			= 	InvalidBackendId;
			sharedData->waited_Vxids[i]->source_vxid.localTransactionId	=	InvalidLocalTransactionId;

			sharedData->sleeping_Vxids[i].backendId 			= 	InvalidBackendId;
			sharedData->sleeping_Vxids[i].localTransactionId	=	InvalidLocalTransactionId;
			
			sharedData->pid_waited[i]			=	(PidCountNode*)ShmemAlloc(sizeof(PidCountNode));
			sharedData->pid_waited[i]->count	=	0;
			sharedData->pid_waited[i]->pid		=	0;
		}
	}
}

...
/*
bool predicion_by_dataid(long inputHash, size_t inputSize, size_t outputSize)
{
    HASHCTL 				hashCT;
    PC3HashKey 				pc3data; 		// key
    PC3HashEntry 			*pc3entry;  	// <key, value>
    bool 					entry_is_found;

    // 初始化哈希表（默认会话周期）
    if (pc3_prediction_hash == NULL)
    {
        hashCT.keysize = sizeof(PC3HashKey);
        hashCT.entrysize = sizeof(PC3HashEntry);
        // 参数1：哈希表名
        // 参数2：哈希表初始大小，建议设小一些，后面不够了，它会自动扩容
        // 参数3：HASHCTL
        // 参数4：HASH_ELEM必选，HASH_BLOBS代表key是二进制结构
        // key中包含各种类型的成员，就要选HASH_BLOBS
        // 如果key只是个字符串，选HASH_STRINGS就够了
        // 详见src/include/utils/hsearch.h
        pc3_prediction_hash = hash_create("pc3 prediction hash",
                                      8, &hashCT,
                                      HASH_ELEM | HASH_BLOBS);
    }

    // 构造待查询的key
    pc3data.inputHash 		= 	inputHash;
	pc3data.inputSize	=	inputSize;
    pc3data.outputSize 	= 	outputSize;

    // 在哈希表中查询指定的key
    // 返回值：未找到返回NULL，找到返回<key, value>
    // 参数1：哈希表
    // 参数2：待查询的key
    // 参数3：动作 HASH_FIND
    // 参数4：当动作为HASH_FIND时，该项设为NULL
    pc3entry = (PC3HashEntry *)hash_search(pc3_prediction_hash, &pc3data, HASH_FIND, NULL);
    if (pc3entry != NULL)
        return pc3entry->res;
    ...
    // 参数2：待存入哈希表中的key
    // 参数3：动作HASH_ENTER
    // 参数4：当动作为HASH_ENTER时，该项为bool *类型
    // 若表中已存在该key，则参数4返回true；不存在该key，参数4返回false，并将<key, NULL>存入表中
    pc3entry = (PC3HashEntry *)hash_search(pc3_prediction_hash, &pc3data, HASH_ENTER, &entry_is_found);
	
    // 给刚刚存入表中的<key, NULL>，赋予相应的value
    if (!entry_is_found)
        pc3entry->res = res;

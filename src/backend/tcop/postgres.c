...
OperatorInfo* addPlanTree(Plan* topPlan)
{
	if(!topPlan) return NULL;

	bool exist_opt = false;

	int opt_index = allOptInfoNum;

	while(opt_index < allOptInfoNum)
	{
		if(currentSQLOpts[opt_index].type == (int)topPlan->type && currentSQLOpts[opt_index].startup_cost == topPlan->startup_cost 
			&& currentSQLOpts[opt_index].total_cost == topPlan->total_cost && currentSQLOpts[opt_index].optId == topPlan->plan_node_id)
		{
			exist_opt = true;
			break;
		}
	...
	if(!exist_opt)
	{
		initializeOperatorInfo(&currentSQLOpts[opt_index], (int)topPlan->type, (double)topPlan->startup_cost, (double)topPlan->total_cost);

		currentSQLOpts[opt_index].optId	=	topPlan->plan_node_id;

		if(topPlan->type >= (int)T_Scan && topPlan->type <= (int)T_CustomScan)
		{
			currentSQLOpts[opt_index].actual_input_rows = topPlan->input_rows; 
		}
		else
		{
			currentSQLOpts[opt_index].actual_input_rows = 0.0;
		}
	...
	if(topPlan->lefttree)
		currentSQLOpts[opt_index].left  = addPlanTree(topPlan->lefttree);
	else
		currentSQLOpts[opt_index].left  = NULL;
	if(topPlan->righttree)
		currentSQLOpts[opt_index].right = addPlanTree(topPlan->righttree);
	else
		currentSQLOpts[opt_index].right = NULL;

	return &currentSQLOpts[opt_index];
}
...
// int
// findConflictedTxnForOCC(VirtualTransactionId current_vxid, TransactionWorkingSet* currentTxn)
// {
// 	PC3HashKey	my_workingset 	= 	currentTxn->workingset;
// 	int 		my_txn_size 	=	currentTxn->workingset.inputSize + currentTxn->workingset.outputSize;
...
// 	int 	index				=	0;
// 	int		min_index 			=	my_txn_size;

// 	TransactionWorkingSet* ptr_transaction = transactionPool->headNode->next;
	
// 	while(ptr_transaction)
// 	{
// 		if( !equalVXID( ptr_transaction->vxid,  current_vxid) && (ptr_transaction->vxid.backendId > 0) && (ptr_transaction->vxid.localTransactionId > 0) && ptr_transaction->begin_time != 0 )
// 		{
// 			if( ptr_transaction->committed && ptr_transaction->commit_time > currentTxn->begin_time && ptr_transaction->begin_time < currentTxn->commit_time)
// 			{
...
// 				// index 	= 	intersectNotEmptyForOCC(current_wokingset, current_writeset, current_size, my_workingset, my_writeset, my_txn_size);
...
// 	if(index != -1)
// 		conflict_num	=	my_txn_size - index;
...
void 
addWaitedVXID(VirtualTransactionId newVxid, int newPid, VirtualTransactionId sourceVxid)
{
	int index 		= 	(newVxid.backendId * 7 + newVxid.localTransactionId * 11) % TXN_SIZE;
	int pid_index	=	newPid % TXN_SIZE;

	int temp 		= 	index;
	int temp_pid 	= 	pid_index;

	while(sharedData->waited_Vxids[index]->pid != 0)
	{
		if( sharedData->waited_Vxids[index]->pid == -1 )
		{
			break;
		}
	
		if( equalVXID(sharedData->waited_Vxids[index]->vxid, newVxid) )
		{
			if(sharedData->waited_Vxids[index]->pid == newPid)
			{
				sharedData->waited_Vxids[index]->isTheLast = true;
				break;
			}
			else 
			{
				sharedData->waited_Vxids[index]->isTheLast = false;
			}	
		}
		...
		if(index == temp)
		{
			if(boolPrint)
			{
				printf("\n-------------[Pid: %d] REPORT ERROR: SharedData WaitedVXID Full! ------------------\n", MyProcPid);
			}
	...
	if(sharedData->waited_Vxids[index]->pid <= 0)
	{
		LWLockAcquire(&sharedData->waited_writeLock, LW_EXCLUSIVE);
		sharedData->waited_Vxids[index]->vxid.backendId = newVxid.backendId;
		sharedData->waited_Vxids[index]->vxid.localTransactionId = newVxid.localTransactionId;
		sharedData->waited_Vxids[index]->source_vxid.backendId	=	sourceVxid.backendId;
		sharedData->waited_Vxids[index]->source_vxid.localTransactionId = sourceVxid.localTransactionId;
		if( sharedData->waited_Vxids[index]->pid == 0)
			sharedData->waited_Vxids[index]->isTheLast = true;
		else if( sharedData->waited_Vxids[index]->pid == -1)
			sharedData->waited_Vxids[index]->isTheLast = false;
		sharedData->waited_Vxids[index]->pid = newPid;
		LWLockRelease(&sharedData->waited_writeLock);
	}

	while(sharedData->pid_waited[pid_index]->pid != 0)
	{
		if(sharedData->pid_waited[pid_index]->pid == newPid)
		{
			sharedData->pid_waited[pid_index]->count++;
			return;
		}

		pid_index++;

		if(pid_index == TXN_SIZE)
			pid_index = 0;

		if(pid_index == temp_pid)
		{
			if(boolPrint) 
			{
				printf("\n-------------[Pid: %d] REPORT ERROR: SharedData PidCount Full! ------------------\n", MyProcPid);
			}
			return;
		}
	}
	
	if(sharedData->pid_waited[pid_index]->pid == 0)
	{
		LWLockAcquire(&sharedData->pid_writeLock, LW_EXCLUSIVE);
		sharedData->pid_waited[pid_index]->pid		=	newPid;
		sharedData->pid_waited[pid_index]->count	=	1;
		LWLockRelease(&sharedData->pid_writeLock);
	}
}

int 
pidCountDecrease(int pid)
{
	Assert(pid != 0);

	int pid_index 	= 	pid % TXN_SIZE;
	int temp		=	pid_index;
	
	while(sharedData->pid_waited[pid_index]->pid != 0)
	{
		if(sharedData->pid_waited[pid_index]->pid == pid)
		{
			if(sharedData->pid_waited[pid_index]->count > 0)
			{
				LWLockAcquire(&sharedData->pid_writeLock, LW_EXCLUSIVE);
				sharedData->pid_waited[pid_index]->count--;
				LWLockRelease(&sharedData->pid_writeLock);
			}
			return sharedData->pid_waited[pid_index]->count;
		}
		pid_index++;
		...
		if(pid_index == temp)
		{
			elog(ERROR, "SharedData PID NOT FOUND!");
		}
	}
...
int*
removeWaitedVxid(VirtualTransactionId vxidToRemove, int* pidSize)
{
	Assert(vxidToRemove.backendId != InvalidBackendId && vxidToRemove.localTransactionId != InvalidLocalTransactionId);

	int* 	pidToWake 		=	(int*)malloc(sizeof(int));

	int backendId 			= 	vxidToRemove.backendId;
	int localTransactionId 	= 	vxidToRemove.localTransactionId;

	int index = (backendId * 7 + localTransactionId * 11) % TXN_SIZE;
	int temp = index;
    ...
    while(sharedData->waited_Vxids[index]->pid != 0)
	{
		...
		if( equalVXID(sharedData->waited_Vxids[index]->vxid, vxidToRemove) )
		{
			int 	pid_to_wakeUp 	= 	sharedData->waited_Vxids[index]->pid;
			// LWLockAcquire(&sharedData->waited_writeLock, LW_EXCLUSIVE);
			sharedData->waited_Vxids[index]->pid 							= 0;
			sharedData->waited_Vxids[index]->vxid.backendId 				= InvalidBackendId;
			sharedData->waited_Vxids[index]->vxid.localTransactionId 		= InvalidLocalTransactionId;
			sharedData->waited_Vxids[index]->source_vxid.backendId 			= InvalidBackendId;
			sharedData->waited_Vxids[index]->source_vxid.localTransactionId = InvalidLocalTransactionId;
			// LWLockRelease(&sharedData->waited_writeLock);
			
			if(pidCountDecrease(pid_to_wakeUp) <= 0 & *pidSize >= 0)
			{
				pidToWake				=	(int*)realloc(pidToWake, (*pidSize+1) * sizeof(int));
				pidToWake[*pidSize] 	= 	pid_to_wakeUp;
				*pidSize				=	*pidSize + 1;
			}
			
			if(sharedData->waited_Vxids[index]->isTheLast)	
			{
				sharedData->waited_Vxids[index]->isTheLast	= false;
				return pidToWake;
			}
		...
		if(index == temp)
		{
			elog(ERROR, "SharedData WaitedVXID NOT FOUND!");
		}
	}
...
void 
addWaitingVXID(VirtualTransactionId newVxid, VirtualTransactionId waitedVxid)
{
	Assert(newVxid.backendId != InvalidBackendId && newVxid.localTransactionId != InvalidLocalTransactionId);

	int backendId 			= 	newVxid.backendId;
	int localTransactionId 	= 	newVxid.localTransactionId;
	
	int index = (backendId * 7 + localTransactionId * 11) % TXN_SIZE;

	int temp = index;
	
	while(sharedData->waiting_Vxids[index]->vxid.backendId > -1 && sharedData->waiting_Vxids[index]->vxid.localTransactionId > 0)
	{
		if(sharedData->waiting_Vxids[index]->vxid.backendId == 0)
		{
			continue;
		}
		
		if( equalVXID(sharedData->waiting_Vxids[index]->vxid, newVxid) )
		{
			if( equalVXID(sharedData->waiting_Vxids[index]->destination_vxid, waitedVxid) )
			{
				return;
			}
			else 
			{
				sharedData->waiting_Vxids[index]->isTheLast		=	false;
			}
		}
		...
		if(index == temp)
		{
			if(boolPrint)
			{
				printf("\n-------------[Pid: %d] REPORT ERROR: SharedData WaitingVXID Full! ------------------\n", MyProcPid);
			}
			return;
		}
	}

	if(sharedData->waiting_Vxids[index]->vxid.backendId <= 0 || sharedData->waiting_Vxids[index]->vxid.localTransactionId <= 0)
	{
		LWLockAcquire(&sharedData->waiting_writeLock, LW_EXCLUSIVE);
		sharedData->waiting_Vxids[index]->vxid.backendId 			= 	newVxid.backendId;	
		sharedData->waiting_Vxids[index]->vxid.localTransactionId 	= 	newVxid.localTransactionId;
		sharedData->waiting_Vxids[index]->destination_vxid.backendId 			= 	waitedVxid.backendId;	
		sharedData->waiting_Vxids[index]->destination_vxid.localTransactionId 	= 	waitedVxid.localTransactionId;
		sharedData->waiting_Vxids[index]->isTheLast					=	true;
		LWLockRelease(&sharedData->waiting_writeLock);
	}
}

void 
removeWaitingVXID(VirtualTransactionId vxidToRemove)
{
	Assert(vxidToRemove.backendId != InvalidBackendId && vxidToRemove.localTransactionId != InvalidLocalTransactionId);
	...
	while(sharedData->waiting_Vxids[index]->vxid.backendId > -1 && sharedData->waiting_Vxids[index]->vxid.localTransactionId > 0)
	{
		...
		if( equalVXID(sharedData->waiting_Vxids[index]->vxid, vxidToRemove) )
		{
			LWLockAcquire(&sharedData->waiting_writeLock, LW_EXCLUSIVE);
			sharedData->waiting_Vxids[index]->vxid.backendId						=	InvalidBackendId;
			sharedData->waiting_Vxids[index]->vxid.localTransactionId				=	InvalidLocalTransactionId;
			sharedData->waiting_Vxids[index]->destination_vxid.backendId			=	InvalidBackendId;
			sharedData->waiting_Vxids[index]->destination_vxid.localTransactionId	=	InvalidLocalTransactionId;
			LWLockRelease(&sharedData->waiting_writeLock);

			if(sharedData->waiting_Vxids[index]->isTheLast)
			{
				sharedData->waiting_Vxids[index]->isTheLast		=	false;
				return;
			}
		...
		if(index == temp)
		{
			elog(ERROR, "SharedData WaitingVXID NOT FOUND!");
		}
	}
}

void 
addSleepingVxid(VirtualTransactionId vxid)
{
	Assert(vxid.backendId != InvalidBackendId && vxid.localTransactionId != InvalidLocalTransactionId);
	
	int index = (vxid.backendId * 7 + vxid.localTransactionId * 11) % TXN_SIZE;

	int temp = index;

	while(sharedData->sleeping_Vxids[index].backendId > 0 && sharedData->sleeping_Vxids[index].localTransactionId > 0)
	{
		if( equalVXID(sharedData->sleeping_Vxids[index], vxid) )
		{
			return;
		}
		...
		if(index == temp)
			elog(ERROR, "Sleeping Transaction Pool Full!");
	}

	LWLockAcquire(&sharedData->sleep_writeLock, LW_EXCLUSIVE);
	sharedData->sleeping_Vxids[index]	=	vxid;
	LWLockRelease(&sharedData->sleep_writeLock);
}

void 
removeSleepingVxid(VirtualTransactionId vxid)
{
	Assert(vxid.backendId != InvalidBackendId && vxid.localTransactionId != InvalidLocalTransactionId);
	...
	while(sharedData->sleeping_Vxids[index].backendId > 0 && sharedData->sleeping_Vxids[index].localTransactionId > 0)
	{
		...
		{
			LWLockAcquire(&sharedData->sleep_writeLock, LW_EXCLUSIVE);
			sharedData->sleeping_Vxids[index].backendId				=	InvalidBackendId;
			sharedData->sleeping_Vxids[index].localTransactionId	=	InvalidLocalTransactionId;
			LWLockRelease(&sharedData->sleep_writeLock);
		}
		...
		if(index == temp)
			elog(ERROR, "Sleeping Transaction NOT FOUND!");
	}
}

bool
checkWaitingStatus(VirtualTransactionId vxid)
{
	Assert(vxid.backendId != InvalidBackendId && vxid.localTransactionId != InvalidLocalTransactionId);
...
void 
reverseWaiting(VirtualTransactionId vxid1, VirtualTransactionId vxid2, int pid1)
{
	Assert(vxid1.backendId != InvalidBackendId && vxid1.localTransactionId != InvalidLocalTransactionId);
	Assert(vxid2.backendId != InvalidBackendId && vxid2.localTransactionId != InvalidLocalTransactionId);
	
	int 	index1 		= 	(vxid1.backendId * 7 + vxid1.localTransactionId * 11) % TXN_SIZE;
	int 	index2 		= 	(vxid2.backendId * 7 + vxid2.localTransactionId * 11) % TXN_SIZE;
	...
	while(sharedData->waited_Vxids[index1]->pid != 0)
	{
		if( equalVXID(sharedData->waited_Vxids[index1]->vxid, vxid1))
		{
			if( equalVXID(sharedData->waited_Vxids[index1]->source_vxid, vxid2) )
			{
				pid2 	= 	sharedData->waited_Vxids[index1]->pid;

				LWLockAcquire(&sharedData->waited_writeLock, LW_EXCLUSIVE);
				if(sharedData->waited_Vxids[index1]->isTheLast)
				{
					if(temp_index1 != -1)
					{
						sharedData->waited_Vxids[temp_index1]->isTheLast = true;
					}
					
					sharedData->waited_Vxids[index1]->pid 		= 	0;
				}
				else 
				{
					sharedData->waited_Vxids[index1]->pid 		= 	-1;
				}

				sharedData->waited_Vxids[index1]->vxid.backendId 					= InvalidBackendId;
				sharedData->waited_Vxids[index1]->vxid.localTransactionId 			= InvalidLocalTransactionId;
				sharedData->waited_Vxids[index1]->source_vxid.backendId 			= InvalidBackendId;
				sharedData->waited_Vxids[index1]->source_vxid.localTransactionId 	= InvalidLocalTransactionId;
				sharedData->waited_Vxids[index1]->isTheLast							= false;
				LWLockRelease(&sharedData->waited_writeLock);
				
				pidCountDecrease(pid2);

				addWaitedVXID(vxid2, pid1, vxid1); /* To add */
		...
		if(index1 == temp1)
		{
			if(boolPrint)
			{
				printf("\n-------------[Pid: %d] REPORT ERROR: SharedData Reversed Waited VXID not FOUND! ------------------\n", MyProcPid);
			}
			return;
		}
	}

	while( sharedData->waiting_Vxids[index2]->vxid.backendId > 0 && sharedData->waiting_Vxids[index2]->vxid.localTransactionId > 0 )
	{
		if( equalVXID(sharedData->waiting_Vxids[index2]->vxid, vxid2))
		{
			if( equalVXID(sharedData->waiting_Vxids[index2]->destination_vxid, vxid1) )
			{
			
				LWLockAcquire(&sharedData->waiting_writeLock, LW_EXCLUSIVE);
				
				if(sharedData->waiting_Vxids[index2]->isTheLast)
				{
					if(temp_index2 != -1)
					{
						sharedData->waiting_Vxids[temp_index2]->isTheLast = true;
					}

					sharedData->waiting_Vxids[index1]->vxid.backendId 					= 	InvalidBackendId;
				}
				else 
				{
					sharedData->waiting_Vxids[index1]->vxid.backendId 					=	0;
				}

				sharedData->waiting_Vxids[index1]->destination_vxid.backendId 			= InvalidBackendId;
				sharedData->waiting_Vxids[index1]->destination_vxid.localTransactionId 	= InvalidLocalTransactionId;
				sharedData->waiting_Vxids[index1]->isTheLast							= false;
				LWLockRelease(&sharedData->waiting_writeLock);
				
				addWaitingVXID(vxid1, vxid2); 
		...
		if(index2 == temp2)
		{
			if(boolPrint)
			{
				printf("\n-------------[Pid: %d] REPORT ERROR: SharedData Reversed Waiting VXID not FOUND! ------------------\n", MyProcPid);
			}
			return;
		}
	}
...
int checkTxnLevel(VirtualTransactionId vxid)
{
	Assert(vxid.backendId != InvalidBackendId && vxid.localTransactionId != InvalidLocalTransactionId);
	
	int backendId			=	vxid.backendId;
	int localTransactionId	=	vxid.localTransactionId;
	int index 				= 	(backendId * 7 + localTransactionId * 11) % TXN_SIZE;
	int temp 				= 	index;
	int level				=	0;
	
    while(sharedData->waiting_Vxids[index]->vxid.backendId > 0 && sharedData->waiting_Vxids[index]->vxid.localTransactionId > 0)
	{
		if( equalVXID(sharedData->waiting_Vxids[index]->vxid, vxid) )
		{
			int temp_level = checkTxnLevel(sharedData->waiting_Vxids[index]->destination_vxid) + 1;
...
// void 
// commitTransactionForOCC(VirtualTransactionId current_vxid)
// {
// 	TransactionWorkingSet* ptr_txn = transactionPool->headNode->next;
// 	while(ptr_txn)
// 	{
// 		if(equalVXID( current_vxid,  ptr_txn->vxid))
// 		{
// 			ptr_txn->committed		=	true;
// 			ptr_txn->commit_time	=	time(NULL);
// 		}
// 		ptr_txn = ptr_txn->next;
// 	}
// }
...
bool checkTxnConflicts(PC3HashKey key1, PC3HashKey key2) 
{
    NewOrderSQLData hashTable[HASH_TABLE_SIZE] = {0};

    for (int i = 0; i < key1.inputSize+key1.outputSize; ++i) 
	{
        int hashIndex = key1.workingset[i].hashValue % HASH_TABLE_SIZE;

        hashTable[hashIndex] = key1.workingset[i];
    }

    for (int j = 0; j < key2.inputSize+key2.outputSize; ++j) 
	{
        int hashIndex = key2.workingset[j].hashValue % HASH_TABLE_SIZE;
        
		if(hashTable[hashIndex].hashValue == key2.workingset[j].hashValue && (hashTable[hashIndex].typeid+key2.workingset[j].typeid)>2 )
		{
			if(boolPrint)
			{
				printf("	[compare 1] key2.workingset[%d] = (%d, %d, %d, %d, %d, %d)\n", j, key2.workingset[j].typeid, key2.workingset[j].tableid, key2.workingset[j].wid, key2.workingset[j].did, key2.workingset[j].cid, key2.workingset[j].iid);
				printf("	[compare 2] hashTable[%d] = (%d, %d, %d, %d, %d, %d)\n", hashIndex, hashTable[hashIndex].typeid, hashTable[hashIndex].tableid, hashTable[hashIndex].wid, hashTable[hashIndex].did, hashTable[hashIndex].cid, hashTable[hashIndex].iid);
			}
...
findConflictedTxn(VirtualTransactionId* result, int* resultSize, VirtualTransactionId current_vxid, TransactionWorkingSet* currentTxn)
{
	PC3HashKey	my_workingset 	= 	currentTxn->workingset;
	int 		my_txn_size 	=	currentTxn->workingset.inputSize	+	currentTxn->workingset.outputSize;
	...
	*resultSize						=	0;
	...
	while(ptr_transaction != NULL)
	{
		if( !equalVXID( ptr_transaction->vxid, current_vxid) && (ptr_transaction->vxid.backendId > 0) && (ptr_transaction->vxid.localTransactionId > 0) )
		{
			...
			foundConflict 	= 	checkTxnConflicts(my_workingset, current_wokingset);
			
            if (foundConflict) 
			{
				if(boolPrint)
				{
					printf("[Vxid:%d/%d] Find Conflicts: %d/%d (inputSize = %d)!\n", current_vxid.backendId, current_vxid.localTransactionId, ptr_transaction->vxid.backendId, ptr_transaction->vxid.localTransactionId, ptr_transaction->workingset.inputSize);
				}

				result 					= 	(VirtualTransactionId*)realloc(result, (*resultSize + 1) * sizeof(VirtualTransactionId));
				result[*resultSize]		=	ptr_transaction->vxid;
			  ...
			  ParamListInfo boundParams)
{
	...
	if(enableME && printExpr && containExpr && plan->planTree)
	{
		addPlanTree(plan->planTree);

		if(plan->subplans)
		{
			ListCell   *lst;

			foreach(lst, plan->subplans)
			{
				Plan *subplan = (Plan *) lfirst(lst);
...
exec_simple_query(const char *query_string)
{
	...
	if(enableME)
	{ 
		int 	type_id 	= 	getSqlStatementType(query_string);  // 1select/2update/0commit/-1rollback
		...
		if(currentSQLExprs)
		{
			free(currentSQLExprs);
		}

		allOptInfoNum			=	0;
		allExprInfoNum			=	0;

		currentSQLOpts			=	(OperatorInfo*)malloc(OPT_LIST_SIZE * sizeof(OperatorInfo));
		currentSQLExprs			= 	(ExprInfo*)malloc(EXPR_MAX_NUM * sizeof(ExprInfo));
		
		if(!currentStatement || strcasecmp(currentStatement, query_string) != 0)
		{
			currentStatement		=	(char*)malloc(strlen(query_string) + 1);
			
			allExprNum				=	0;
			allExprList				=	(int*)malloc(EXPR_MAX_NUM * sizeof(int));
			currentExprJITLevels	=	(int*)malloc(EXPR_MAX_NUM * sizeof(int));

			previousLevelsNum		=	0;
			previousExprJITLevels	=	(long long*)malloc(LEVELS_NUM * sizeof(long long));

			strcpy(currentStatement, query_string);

			currentStatementExecNum	=	0;

			currentLevelsCount		=	0;
		}
		else
		{
			currentStatementExecNum++;

			if(currentStatementExecNum == 1)
			{
				for(int i = 0; i < allExprNum; i++)
				{
					currentExprJITLevels[i]	=	0;
				}

				currentLevelsCount++;
			}
			else if(currentLevelsCount >= Reserved1)
			{
				// Get JIT levels for every JITed expression

				long long current_hash = 0;

				bool valid_levels	=	false;
				...
				while(!valid_levels)
				{
					...
					if(round++ >= 10)
					{
						printf("10 rounds of Invalid levels!\n");
					...
					if(random_num < 9)
					{
						for(int i = 0; i < allExprNum; i++)
						{
							...
							currentExprJITLevels[i]	=	random_num;

							current_hash = (current_hash*4 + random_num) % (LLONG_MAX/4);
						}
					}
					else
					{
						for(int j = 0; j < allExprNum; j++)
						{
							srand(time(NULL) + rand());

							int random_num =  3 * (rand() % 2); 

							currentExprJITLevels[j]	=	random_num;

							current_hash = (current_hash*4 + random_num) % (LLONG_MAX/4);
						}
					}
				
					for(int j = 0; j < previousLevelsNum; j++)
					{
						if(previousExprJITLevels[j] == current_hash && current_hash != 0LL)
						{
							valid_levels	=	false;
				...
				previousExprJITLevels[previousLevelsNum++]	=	current_hash;
	...
	if(enableME)
	{
		if(printExpr && containExpr)
		{
			const char* rFilename = "/data/report.csv";

			FILE *rFile = fopen(rFilename, "a+"); 
			...
			{
				fprintf(rFile, "\"%s\",%s", query_string, databaseName);

				for(int i=0; i<allExprInfoNum; i++)
				{
					ExprInfo* tempExpr = &currentSQLExprs[i];

					if(currentSQLExprs[i].expression_id != 0)
					{
						if(currentSQLExprs[i].op)
						{
							fprintf(rFile, ",%d,%ld,%d,%s,%.6lf,%.6lf", currentSQLExprs[i].expression_id, currentSQLExprs[i].count, 
								currentSQLExprs[i].op->optId, getNodeTagName((NodeTag)currentSQLExprs[i].op->type), currentSQLExprs[i].op->startup_cost, 
								currentSQLExprs[i].op->total_cost);
						}
						else
						{
							fprintf(rFile, ",%d,%ld,0,T_Invalid,0.0,0.0", tempExpr->expression_id, tempExpr->count);
						}
					}
				}
			...
			const char* oFilename = "/data/operator.csv";
			...
			{
				for(int j=0; j<allOptInfoNum; j++)
				{
					bool   hasLeft				=	false;
					bool   hasRight				=	false;

					fprintf(oFile, "%d,%s,%.6lf,%.6lf,%.0lf,%.0lf,%.0lf,%.6lf,%.6lf,", currentSQLOpts[j].optId, getNodeTagName((NodeTag)currentSQLOpts[j].type), currentSQLOpts[j].startup_cost, 
						currentSQLOpts[j].total_cost, currentSQLOpts[j].actual_input_rows, currentSQLOpts[j].actual_output_rows, 
						currentSQLOpts[j].filtered_rows, currentSQLOpts[j].actual_startup_time, currentSQLOpts[j].actual_total_time);

					if(currentSQLOpts[j].left)
					{
						fprintf(oFile, "%d,%s,%.6lf,%.6lf,", currentSQLOpts[j].left->optId, getNodeTagName((NodeTag)currentSQLOpts[j].left->type), currentSQLOpts[j].left->startup_cost, currentSQLOpts[j].left->total_cost);
					}
					else
					{
						fprintf(oFile, "0,T_Invalid,0.0,0.0,");
					}

					if(currentSQLOpts[j].right)
					{
						fprintf(oFile, "%d,%s,%.6lf,%.6lf\n", currentSQLOpts[j].right->optId, getNodeTagName((NodeTag)currentSQLOpts[j].right->type), currentSQLOpts[j].right->startup_cost, currentSQLOpts[j].right->total_cost);
					}
					else
					{
						fprintf(oFile, "0,T_Invalid,0.0,0.0\n");
			...
			FILE *eeFile = fopen(eeFilename, "w+"); 
			...
			{	
				for(int k=0; k<EXPR_LIST_SIZE; k++)
				{
					if(exprTypes[k].expression_id != 0)
					{
						fprintf(eeFile, "表达式编号 %d:", exprTypes[k].expression_id);

						for(int m=0; m<exprTypes[k].step_lens; m++)
						{
							fprintf(eeFile, " %s", getExprEvalOpName((ExprEvalOp)exprTypes[k].steps[m]));
						}
			 ...
			 const char *username)
{
	...
	databaseName	=	(char*)malloc((strlen(dbname) + 1) * sizeof(char));

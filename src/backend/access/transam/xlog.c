...
ExprSize(void)
{
	...
	size = EXPR_LIST_SIZE * sizeof(ExprSteps);

	size = add_size(size, EXPR_MAX_NUM * sizeof(ExprInfo));

	size = add_size(size, OPT_LIST_SIZE * sizeof(OperatorInfo));

	size = add_size(size, 1024 * sizeof(int));

	size = add_size(size, LEVELS_NUM * sizeof(long long));

	return size;
}

// 解析文件并将表达式信息存储到 exprList 中
void parse_expression_file(const char* filename) 
{
	exprTypes	=	initializeExprStepsList();
    ...
    while (fgets(line, sizeof(line), file)) 
	{
        ...
        // 获取表达式编号
        if (strncmp(token, "表达式编号", 12) == 0) 
		{
            int expression_id = (int)atoi(strtok(NULL, ":"));
			...
			{
				char* new_token		=	trim_token(token);

                int step_value 		= 	getExprEvalOpValue(new_token);
                steps[step_count++] = 	step_value;

                token 				= 	strtok(NULL, " ");
            }

            if(add_expression(step_count, steps))
				continue;
        }	
    }

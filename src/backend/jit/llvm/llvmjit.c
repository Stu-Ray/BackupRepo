...
static void llvm_optimize_module_select(LLVMModuleRef module, bool flag_opt3, bool flag_inline);
...
llvm_release_context(JitContext *context)
{
	...
	if (llvm_context->module_opt)
	{
		LLVMDisposeModule(llvm_context->module_opt);
		llvm_context->module = NULL;
	}

	if (llvm_context->module_jit)
	{
		LLVMDisposeModule(llvm_context->module_jit);
		llvm_context->module_jit = NULL;
	}

	if (llvm_context->module_inline)
	{
		LLVMDisposeModule(llvm_context->module_inline);
		llvm_context->module_inline = NULL;
	}
...
/*
 * #RAIN: Return module which may be modified, e.g. by creating new functions.
 */
LLVMModuleRef
llvm_mutable_module_select(LLVMJitContext *context, ModuleType type)
{
	llvm_assert_in_fatal_section();

	/*
	 * If there's no selected in-progress module, create a new one.
	 */
	if (!context->module_jit && !context->module_inline && !context->module_opt && !context->module)
	{
		context->compiled = false;
		context->module_generation = llvm_generation++;
	}
	...
	if(type == MODULE_JIT)
	{
		if (!context->module_jit)
		{
			context->module_jit = LLVMModuleCreateWithName("pg_jit");
			LLVMSetTarget(context->module_jit, llvm_triple);
			LLVMSetDataLayout(context->module_jit, llvm_layout);
		}
		return context->module_jit;
	}
	else if(type == MODULE_INLINE)
	{
		if (!context->module_inline)
		{
			context->module_inline = LLVMModuleCreateWithName("pg_inline");
			LLVMSetTarget(context->module_inline, llvm_triple);
			LLVMSetDataLayout(context->module_inline, llvm_layout);
		}
		return context->module_inline;
	}
	else if(type == MODULE_OPT)
	{
		if (!context->module_opt)
		{
			context->module_opt = LLVMModuleCreateWithName("pg_opt");
			LLVMSetTarget(context->module_opt, llvm_triple);
			LLVMSetDataLayout(context->module_opt, llvm_layout);
		}
		return context->module_opt;
	}
	else
	{
		if (!context->module)
 ...
 */
static void
llvm_optimize_module_select(LLVMModuleRef module, bool flag_opt3, bool flag_inline)
{
	...
	if (flag_opt3)  /*context->base.flags & PGJIT_OPT3*/
		compile_optlevel = 3;
	else
		compile_optlevel = 0;
	...
	if (flag_inline && !flag_opt3)
		LLVMAddFunctionInliningPass(llvm_mpm);
	LLVMRunPassManager(llvm_mpm, module);
...
llvm_compile_module(LLVMJitContext *context)
{
	...
	if(context->module_inline)
	{
		llvm_inline(context->module_inline);
	}
	
	if(context->module_opt) 
	{
		llvm_inline(context->module_opt);
	}
	
	INSTR_TIME_SET_CURRENT(endtime);
	INSTR_TIME_ACCUM_DIFF(context->base.instr.inlining_counter, endtime, starttime);

	if (jit_dump_bitcode && context->module_jit)
	{
		char	   *filename;
		filename = psprintf("module_jit_%u.%zu.bc",
							MyProcPid,
							context->module_generation);
		LLVMWriteBitcodeToFile(context->module_jit, filename);
		pfree(filename);
	}

	if (jit_dump_bitcode && context->module_inline)
	{
		char	   *filename;
		filename = psprintf("module_inline_%u.%zu.bc",
							MyProcPid,
							context->module_generation);
		LLVMWriteBitcodeToFile(context->module_inline, filename);
		pfree(filename);
	}

	if (jit_dump_bitcode && context->module_opt)
	{
		char	   *filename;
		filename = psprintf("module_opt_%u.%zu.bc",
							MyProcPid,
							context->module_generation);
		LLVMWriteBitcodeToFile(context->module_opt, filename);
		pfree(filename);
	}
	...
	if(context->module_jit)
		llvm_optimize_module_select(context->module_jit, false, false);
	if(context->module_inline)
		llvm_optimize_module_select(context->module_inline, false, true);
	if(context->module_opt)
		llvm_optimize_module_select(context->module_opt, true, true);
...
#if LLVM_VERSION_MAJOR > 11
	{
		...
		LLVMOrcThreadSafeModuleRef ts_module2;
		LLVMOrcThreadSafeModuleRef ts_module3;
		LLVMOrcThreadSafeModuleRef ts_module4;
		LLVMErrorRef error;
		LLVMErrorRef error2;
		LLVMErrorRef error3;
		LLVMErrorRef error4;
		LLVMOrcJITDylibRef jd = LLVMOrcLLJITGetMainJITDylib(compile_orc);  /*compile_orc -> llvm_opt3_orc/llvm_opt0_orc*/
		LLVMOrcJITDylibRef jd2 = LLVMOrcLLJITGetMainJITDylib(llvm_opt0_orc); 
		LLVMOrcJITDylibRef jd3 = LLVMOrcLLJITGetMainJITDylib(llvm_opt3_orc);
		...
		{
			// printf("[LLVM_JIT]\n");
			ts_module2 = LLVMOrcCreateNewThreadSafeModule(context->module_jit, llvm_ts_context);
			handle->lljit = llvm_opt0_orc; /*compile_orc*/
			handle->resource_tracker = LLVMOrcJITDylibCreateResourceTracker(jd2);
			context->module_jit = NULL; /* will be owned by LLJIT */
			error2 = LLVMOrcLLJITAddLLVMIRModuleWithRT(llvm_opt0_orc, 
														handle->resource_tracker,
														ts_module2);
			if (error2)
				elog(ERROR, "failed to JIT module: %s",
						llvm_error_message(error2));
			handle->lljit = llvm_opt0_orc; 
		}

		if(context->module_inline)
		{
			// printf("[LLVM_INLINE]\n");
			ts_module3 = LLVMOrcCreateNewThreadSafeModule(context->module_inline, llvm_ts_context);
			handle->lljit = llvm_opt0_orc; 
			handle->resource_tracker = LLVMOrcJITDylibCreateResourceTracker(jd2);
			context->module_inline = NULL; /* will be owned by LLJIT */
			error3 = LLVMOrcLLJITAddLLVMIRModuleWithRT(llvm_opt0_orc, 
													handle->resource_tracker,
													ts_module3);
			if (error3)
				elog(ERROR, "failed to JIT module: %s",
					llvm_error_message(error3));
			handle->lljit = llvm_opt0_orc; 
		}
		...
		{
			// printf("[LLVM_OPT]\n");
			ts_module4 = LLVMOrcCreateNewThreadSafeModule(context->module_opt, llvm_ts_context);
			handle->lljit = llvm_opt3_orc; 
			handle->resource_tracker = LLVMOrcJITDylibCreateResourceTracker(jd3);
			context->module_opt = NULL; /* will be owned by LLJIT */
			error4 = LLVMOrcLLJITAddLLVMIRModuleWithRT(llvm_opt3_orc, 
													handle->resource_tracker,
													ts_module4);
			if (error4)
				elog(ERROR, "failed to JIT module: %s",
					llvm_error_message(error4));				
			handle->lljit = llvm_opt3_orc; 			  
		}
		...
		// handle->lljit = compile_orc; /*compile_orc*/
		// handle->resource_tracker = LLVMOrcJITDylibCreateResourceTracker(jd);

		// context->module = NULL; /* will be owned by LLJIT */
		// error = LLVMOrcLLJITAddLLVMIRModuleWithRT(compile_orc, /*compile_orc*/
		// 										  handle->resource_tracker,
		// 										  ts_module);
	...
	context->module = NULL;
	context->module_jit = NULL;
	context->module_inline = NULL;
	context->module_opt = NULL;

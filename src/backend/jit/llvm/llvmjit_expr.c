...
llvm_compile_expr(ExprState *state)
{
	...
	ModuleType module_type =  (ModuleType)state->parent->jit_level;	

	// if(enableME && printJITExpr)
	// {
	...
	// 	printf("[MOD SELECTED] Level: %d [Expression_ID] %d\n", module_type, current_expr->expression_id);
	...
	if(module_type	== MODULE_DEF)
		mod = llvm_mutable_module(context);
	else
		mod = llvm_mutable_module_select(context, module_type);  

...
jit_compile_expr(struct ExprState *state)
{
	...
	if(enableME)
	{
		...
		if(current_expr)
		{
			...
			if(!found_expr)
			{
				state->parent->list_index 				= 	allExprNum;
				allExprList[allExprNum]					=	expressionId;
				currentExprJITLevels[allExprNum++]		=	0;
			}
			...
			{
				printf("[Error] current_expr is null! \n");
			}
		}
	}

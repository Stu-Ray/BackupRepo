...
ExecInterpExpr(ExprState *state, ExprContext *econtext, bool *isnull)
{
	...
	if(enableME && printExpr && containExpr && countExpr && state)
	{
		if(state->parent)
		{
			if(state->parent->plan)
			{
				...
				int		opt_id		=	(int)state->parent->plan->plan_node_id;
				int		opt_type	=	(int)state->parent->plan->type;
				double	opt_scost	=	(double)state->parent->plan->startup_cost;
				double	opt_tcost 	=	(double)state->parent->plan->total_cost;
				...
				if(currentSQLExprs)
				{
					...
					while(expr_info_index < allExprInfoNum)
					{
						if(currentSQLExprs[expr_info_index].hash == eHash && currentSQLExprs[expr_info_index].step_lens == steps_length)
						{
							...
							{
								if(currentSQLExprs[expr_info_index].op->type == opt_type && currentSQLExprs[expr_info_index].op->startup_cost == opt_scost && currentSQLExprs[expr_info_index].op->total_cost == opt_tcost && currentSQLExprs[expr_info_index].op->optId == opt_id)
								{
									currentSQLExprs[expr_info_index].count++;

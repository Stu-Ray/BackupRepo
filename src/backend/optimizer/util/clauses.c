...
get_agg_clause_costs_walker(Node *node, get_agg_clause_costs_context *context)
{
	...
	if (IsA(node, Aggref))
	{
		...
		/*#RAIN - temp costs*/
		// QualCost 	temp_transCost 	= context->costs->transCost;
		// QualCost 	temp_finalCost 	= context->costs->finalCost;
		// int 		temp_numAggs	= context->costs->numAggs; 
		...
		// printf("[GET CURRENT COST] transCost.startup = %lf, transCost.per_tuple = %lf, finalCost.startup = %lf, finalCost.per_tuple = %lf\n", costs->transCost.startup, costs->transCost.per_tuple, costs->finalCost.startup, costs->finalCost.per_tuple);

			...
			ExplainState *es)
{
	...
	if(enableME && printExpr && containExpr)
	{

		if(planstate->instrument)
		{
			OperatorInfo* opt = findOperatorInfo(planstate->plan->plan_node_id, planstate->plan->type, planstate->plan->startup_cost, planstate->plan->total_cost);

			if(opt)
			{
				opt->actual_output_rows 	= 	planstate->instrument->ntuples + planstate->instrument->ntuples2;
				opt->actual_startup_time	=	planstate->instrument->startup;
				opt->actual_total_time		=	planstate->instrument->total;
				opt->filtered_rows			=	planstate->instrument->nfiltered1 + planstate->instrument->nfiltered2;
				opt->loops					=	planstate->instrument->nloops;
			}
